...
func setup(initial_position: Vector2, target_position: Vector2, weapon_data: WeaponData = null, flipAttack = false):
	...
	var wave_pos = get_parent().get_node_or_null("WavePosition")
	if wave_pos:
		#print('whip position: ', whip_pos.position)
		position = wave_pos.position
	else:
	...
	z_index = -1 #fica atrás do player
...
func enable_attack():
	...
	if $AnimationPlayer.has_animation("wavePass"):
		$AnimationPlayer.play("wavePass")

func _on_animation_player_animation_finished(anim_name: StringName) -> void:
	if anim_name == "wavePass":
		queue_free()
